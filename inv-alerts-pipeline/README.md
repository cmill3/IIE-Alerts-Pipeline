...
```bash
inv-alerts-pipeline$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
inv-alerts-pipeline$ sam local start-api
inv-alerts-pipeline$ curl http://localhost:3000/classify_digit
```
...
```bash
inv-alerts-pipeline$ sam logs -n InferenceFunction --stack-name "inv-alerts-pipeline" --tail
```
...
```bash
sam delete --stack-name "inv-alerts-pipeline"
```
